var ExamModule = (function () {
    ...
    function eventBind() {
        ...
        $root.on('click', '#submit', submitExam);//다음 강의로 이동
        $root.on('click', '#returnExam', submitExam);//문제 다시 풀기
    }
    ...
    function postQuestionSubmit(obj) {
        ...
        for (var i = 0; i < EXAM_ANSWER.length; i++) {//정답 갯수 
            ...
            if (EXAM_ANSWER[i] == data_obj['answers[' + i + ']']) {
                ...
                if(i <= 9)
                {
                    ...
                    answer_html_arr.push(answer_str+'번째 문항 오답입니다')
                }
                else
                {
        ...
        console.log("오답 번호",WRONG_ANSWER);

        // console.log("answer_str",answer_str);
        // document.querySelector('.js-pop_exam h1 span').innerHTML = answer_cnt
        var html = ''

        // 최소 정답수량 보다 작을 경우    // 선택한 답이 정답이 아닐 경우
        if(answer_cnt<ANSWER_MIN_CNT){  //data_obj['answers[' + answer_cnt + ']']!=EXAM_ANSWER[i]
            html += '<h1>오답입니다.</h1>'
            // var answer_html_arr = []
            ...
            html += '<p>'+answer_html_arr.join('<br>') + '<br>문제를 다시 풀어주세요</p>'
            html += '<button id="returnExam" data-type="0" class="btn">다시 풀기</button>'

            document.querySelector('.js-pop_exam_text').innerHTML = html

            MainModule.popVerticalMiddle(0);//오답일 경우 팝업창만 띄우기
            ...
            html += '<p>'+answer_html_arr.join('<br>') + '<br>다음 강의로 이동해주세요</p>'
            html += '<button  id="submit" data-type="0" class="btn">다음 강의로 이동</button>'

            document.querySelector('.js-pop_exam_text').innerHTML = html

            //서버에 보내기(결과값 저장)/ 통과일때만 서버에 저장 됨
            var settings = {
    ...
    function submitExam() {
        if (!is_next) {
            location.reload();//문서 재전송 , 문제 다시풀기 기능과 동일
        } else {
            location.replace('./list.html')
        }
    }
