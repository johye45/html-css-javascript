    ...
    function sendPhoneNumber(){
        ...
        for(var i=0;i<CHECK_VALUE_ARR.length;i++){
            ...
            if(target==null||target==undefined||target==''){
                alert(ALERT_TEXT_OBJ.basic[0]);//잘못된 접근
                return false;
            }
            if(target.value.replace()===''){
                alert(ALERT_TEXT_OBJ.login[i]);//이름, 핸드폰 알림창
                target.focus();
                return false;
            }
        ...
        //지정된 접속자가 아니고 서버시간이 시작시간보다 작거나 서버시간이 끝나는 시간보다 크거나 하면 접속 불가!
        ...
        document.querySelector('.js-token_time').innerHTML="";//token시간 공백
        clearInterval(TOKEN_TIMER);//setInterval로 반복하는 걸 멈춤
        token_time = TOKEN_LIMIT_TIME;
        updateTokenTime();//토큰 시간 
        document.getElementById("sendToken").setAttribute('disabled', true);//(attributeName, attributeValue)
        var settings= {
        ...
        $.ajax(settings).done(function(response){
            console.log(response);
            alert("070 번호 수신거부시, 문자 수신이 어려울 수 있으니 \n 문자가 오지 않는 참가자분은 확인해주시길 바랍니다.");
            var input_els = document.querySelectorAll('.login_form input[type="text"]');
            for(var i=0; i,input_els.length;i++){
                //eq(i) : 선택하 ㄴ요소 중에서 전달받은 인덱스에 해당하는 요소를 선택함
                //attr() : HTML attribute값이 모두 String으로 넘어옴
                $('.login_form input[type="text"]').eq(i).attr('readonly', 'readonly');//readonly로 해도되고 true로 해도됨
            }
            code_el = document.getElementById("token");//인증번호
            login_btn_el = document.getElementById("submit");//로그인
